...
def trigger_sending(data, port):
    value = hex(int(data, 8))
    try: 
        ext_port = parallel.Parallel(port=port)
        ext_port.setData(value)
        return print('successfully send trigger: ' + value + ', oct: ' + data)
    except:
        return print('No device! Planned to send trigger: ' + value + ', oct: ' + data)
